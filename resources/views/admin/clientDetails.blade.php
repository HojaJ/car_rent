            ...
            <div class="md:w-1/4 md:border-r border-gray-900 flex flex-col gap-8">
                ...
                <div class="text-center md:text-start">
                    ...
                    <h2 class="text-lg font-medium text-green-600"><span
                            class="text-lg font-car font-normal text-gray-500">{{ __('Active Reservations') }}: </span>{{ $user->reservations->where('status', 'Active')->count() }}</h2>
                    <h2 class="text-lg font-medium text-yellow-300"><span
                            class="text-lg font-car font-normal text-gray-500">{{ __('Pending Reservations') }}: </span>{{ $user->reservations->where('status', 'Pending')->count() }}</h2>
                    <h2 class="text-lg font-medium text-Black"><span
                            class="text-lg font-car font-normal text-gray-500">{{ __('Ended Reservations') }}: </span>{{ $user->reservations->where('status', 'Ended')->count() }}</h2>
                    <h2 class="text-lg font-medium text-red-600"><span
                            class="text-lg font-car font-normal text-gray-500">{{ __('Canceled Reservations') }}: </span>{{ $user->reservations->where('status', 'Canceled')->count() }}</h2>
                    <h2 class="text-lg font-medium text-pr-400"><span
                            class="text-lg font-car font-normal text-gray-500">{{ __('Total Reservations') }}: </span>{{ $user->reservations->count() }}</h2>
