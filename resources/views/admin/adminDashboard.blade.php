                ...
                <div class="container px-6 mx-auto grid mb-32 ">
                    ...
                    <div class="mt-12">
                        <div class="flex align-middle justify-center">
                            ...
                            <p class="my-2 mx-8  p-2 font-car font-bold text-gray-600 text-lg ">{{ __('Reservation') }}</p>
                            <hr class=" mt-8 h-0.5 w-1/2 bg-pr-500">
                            <hr>
                        </div>
                            ...
                            <table class="w-full whitespace-no-wrap overflow-scroll table-auto">
                                <thead>
                                    <tr
                                        ...
                                        <th class="px-4 py-3 w-48">{{ __('Car') }}</th>
                                        <th class="px-4 py-3 w-24">{{ __('Start Time') }}</th>
                                        <th class="px-4 py-3 w-24">{{ __('Finish Time') }}</th>
                                        <th class="px-4 py-3">{{ __('Duration') }}</th>
                                        <th class="px-4 py-3 w-26">{{ __('Remaining days') }}</th>
                                ...
                                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                                    ...
                                    @forelse ($reservations as $reservation)
                                        <tr class="text-gray-700 dark:text-gray-400">
                                            ...
                                            <td class="px-4 py-3 text-sm">
                                                {{ $reservation->car->price_per_day * $reservation->days }} TMT
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-sm ">
                                                @if ($reservation->status == 'Pending')
                                                    <span
                                                        class="p-2 text-white rounded-md bg-yellow-300" style="white-space: nowrap">{{ @trans($reservation->status) }}</span>
                                                @elseif ($reservation->status == 'Ended')
                                                    <span
                                                        class="p-2 text-white rounded-md bg-black " style="white-space: nowrap">{{ @trans($reservation->status) }}</span>
                                                @elseif ($reservation->status == 'Active')
                                                    <span
                                                        class="p-2 text-white rounded-md bg-green-500 px-4" style="white-space: nowrap">{{ @trans($reservation->status) }}</span>
                                                @elseif ($reservation->status == 'Canceled')
                                                    <span
                                                        class="p-2 text-white rounded-md bg-red-500 " style="white-space: nowrap">{{ @trans($reservation->status) }}</span>
                                                @endif
                                            </td>
